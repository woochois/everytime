...
<body>
	...
	<div id="submenu">
		<div class="wrap">
			...
			<div class="group">
				<ul>
					<li><a href="/board/freeboard/freeBoardList" data-id="389131"
					...
					<li><a href="/board/promotionboard/promotionBoardList"
						data-id="367503">홍보게시판</a></li>
	...
	<div id="container" class="article">
		...
		<div class="wrap articles">
			...
			<form action="/board/promotionboard/pbinsert" class="write">
					...
					<div class="panel panel-default">
						...
						<div id="newArticle" class="panel-collapse collapse"
							...
							<div class="panel-body">
								<div>
									<input type="hidden" name="pnickname"
										value="${member.nickname}" /><br /> <input type="text"
										name="ptitle" required="required" autofocus="autofocus"
										placeholder="글 제목 입력" class="title" />
								</div>
								<div>
									<textarea name="pcontents" cols="30" rows="10"
...
4. 기타 약관 및 현행법에 어긋나는 행위"></textarea>
			...
			<c:forEach var="promotionBoardList" items="${promotionBoardList}">
				<article>
					<a class="article"
						href="/board/promotionboard/promotionBoardListView/pbno/${promotionBoardList.pbno}/pageNum/${pb.currentPage}">
						<h2 class="medium">${promotionBoardList.ptitle }</h2>
						<p class="small">${promotionBoardList.pcontents }</p> <time
							class="small">${promotionBoardList.pregdate }</time>
						<h3 class="small">${promotionBoardList.pnickname }</h3>
						<ul class="status">
							<li title="공감" class="vote"><svg width="1em" height="1em"
							...
							</svg>${promotionBoardList.precommendation }</li>
							<li title="댓글" class="comment"><svg width="1em" height="1em"
									viewBox="0 0 16 16" class="bi bi-chat" fill="currentColor"
									xmlns="http://www.w3.org/2000/svg">
  								<path fill-rule="evenodd"
										d="M2.678 11.894a1 1 0 0 1 .287.801 10.97 10.97 0 0 1-.398 2c1.395-.323 2.247-.697 2.634-.893a1 1 0 0 1 .71-.074A8.06 8.06 0 0 0 8 14c3.996 0 7-2.807 7-6 0-3.192-3.004-6-7-6S1 4.808 1 8c0 1.468.617 2.83 1.678 3.894zm-.493 3.905a21.682 21.682 0 0 1-.713.129c-.2.032-.352-.176-.273-.362a9.68 9.68 0 0 0 .244-.637l.003-.01c.248-.72.45-1.548.524-2.319C.743 11.37 0 9.76 0 8c0-3.866 3.582-7 8-7s8 3.134 8 7-3.582 7-8 7a9.06 9.06 0 0 1-2.347-.306c-.52.263-1.639.742-3.468 1.105z" />
							</svg>${promotionBoardList.pcommentCount }</li>
							<li title="조회수"><svg width="1em" height="1em"
							...
							</svg> ${promotionBoardList.preadcount }</li>
						</ul>
					</a>
			...
			<!-- search -->
			<form action="/board/promotionboard/promotionBoardList/pageNum/1"
				method="get">
				<select name="search">
					<c:forTokens var="item" items="all,pnickname,ptitle,pcontents"
						delims="," varStatus="i">
						<c:if test="${promotionboard.search==item }">
					...
					value="${promotionboard.keyword }"> <input type="submit"
					value="확인">
			</form>
				...
				<ul class="pagination">
					<c:if test="${pb.startPage > pb.pagePerBlock}">
						<li class="page-item"><a class="page-link"
							href="/board/promotionboard/promotionBoardList/pageNum/1?search=${promotionboard.search}&keyword=${promotionboard.keyword}">
						...
						<li class="page-item"><a class="page-link"
							href="/board/promotionboard/promotionBoardList/pageNum/${pb.startPage-1}?search=${promotionboard.search}&keyword=${promotionboard.keyword}">
								<svg width="1em" height="1em" viewBox="0 0 16 16"
									class="bi bi-chevron-compact-left" fill="currentColor"
									xmlns="http://www.w3.org/2000/svg">
  									<path fill-rule="evenodd"
					...
					<c:forEach var="i" begin="${pb.startPage}" end="${pb.endPage}">
						<c:if test="${i==pb.currentPage}">
							<li class="active page-item"><a class="page-link"
								href="/board/promotionboard/promotionBoardList/pageNum/${i}?search=${promotionboard.search}&keyword=${promotionboard.keyword}">${i}</a></li>
						</c:if>
						<c:if test="${i!=pb.currentPage}">
							<li class="page-item"><a class="page-link"
								href="/board/promotionboard/promotionBoardList/pageNum/${i}?search=${promotionboard.search}&keyword=${promotionboard.keyword}">${i}</a></li>
					...
					<c:if test="${pb.endPage < pb.totalPage }">
						<li class="page-item"><a class="page-link"
							href="/board/promotionboard/promotionBoardList/pageNum/${pb.endPage+1}?search=${promotionboard.search}&keyword=${promotionboard.keyword}">
								<svg width="1em" height="1em" viewBox="0 0 16 16"
									class="bi bi-chevron-compact-right" fill="currentColor"
									xmlns="http://www.w3.org/2000/svg">
  									<path fill-rule="evenodd"
						...
						<li class="page-item"><a class="page-link"
							href="/board/promotionboard/promotionBoardList/pageNum/${pb.totalPage}?search=${promotionboard.search}&keyword=${promotionboard.keyword}">
								<svg width="1em" height="1em" viewBox="0 0 16 16"
		...
		<div class="rightside">
			<div class="card">
				<div class="board">
					...
					<a class="article" href="#"><p class="title">예과 2학년</p>
						<p class="small">화이팅!</p>
						<h4>자유게시판</h4>
						<ul class="status">
						...
						<hr></a><a class="article" href="#"><p class="title">타임
							매직!</p>
						<p class="small">
