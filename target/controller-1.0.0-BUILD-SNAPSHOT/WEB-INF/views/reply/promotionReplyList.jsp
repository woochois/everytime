...
<script type="text/javascript">
	function prDelete(prbno, prrno) {
		var sendData = 'pbno=' + prbno + '&prrno=' + prrno
				+ '&pageNum=${pageNum}';
		$.post('/prDelete', sendData, function(data) {
			alert("삭제되었습니다");
			$('#promotionReplyListDisp').load(
					window.location.href + "#promotionReplyListDisp");
		});
	}
			...
			<table class="table table-striped table-hover">
				...
				<c:forEach var="promotionreply" items="${prList}">
					<tr>
						...
						<td>
							<form action="/prrRec/pbno/${pbno}/pageNum/${pageNum}"
								id="precform">
								<input type="hidden" name="prrpno"
									value="${promotionreply.prrno}" /> <input type="hidden"
									name="pbno" value="${promotionreply.prbno}" />
								<button type="submit" id="precbutton">
									댓글추천
						...
						<td>
							<!-- 댓글 작성자와 로그인 회원의 닉네임이 같으면 --> <c:if
								...
								<a
									href="/promotionReplyDelete/prbno/${promotionreply.prbno }/prrno/${promotionreply.prrno }/pageNum/${pageNum}"
